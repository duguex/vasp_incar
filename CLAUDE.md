...
**Stage 1 - `resolve_tag()`:** exact → fuzzy (difflib) → substring
**Stage 2 - `query_tag()`:** assembles wiki info + real INCAR configs + stats + co-occurrence

**`hybrid_search()`:** BM25 (tantivy, Rust, 15.4k⭐) + semantic (sentence-transformers, 18.8k⭐) → RRF fusion

Runtime flow:
```
search("energy cutoff")
  → exact name match → none
  → hybrid_search("energy cutoff")
  → BM25 matches "energy cutoff" in ENCUT description → ENCUT ranked #1
  → {"info": ..., "configs": ..., "stats": ..., "related": ...}
  
search("convergence")
  → no exact match → hybrid_search("convergence")
  → tantivy BM25 + sentence-transformers BGE-small → RRF → top-N results
  → each result has score, type, id
```
...
- **T3 — hybrid_search:** BM25 (tantivy) + semantic (sentence-transformers BGE-small). Used for agent-style natural language queries and when T1/T2 miss. Latency: ~30ms in MCP server (model cached), ~15s in CLI (model load per invocation).
...
**CLI vs MCP server behavior:**
- CLI (`python3 -m vasp_query search`): T1/T2 handle most queries instantly. T3 hybrid search works but incurs ~15s model load per invocation.
- MCP server (long-lived process): model stays resident, T3 hybrid search runs in ~30ms. The `search_tags` tool uses the same tiered pipeline.
