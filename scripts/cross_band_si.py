#!/usr/bin/env python3
"""Compare KS orbital **energies** (eigenvalues) for Si on VASP vs OpenMX.

Protocol
--------
- Same geometry: cubic diamond Si₈ @ a0 = 5.431 Å (fixed)
...
A0 = 5.431
N_ATOM = 8
# Si PAW/PAO: 4 valence e− per atom → 32 electrons → 16 occupied bands (spin-unpolarized)
N_OCC = 16
...
def write_si_poscar(path: Path) -> None:
    from ase.build import bulk
    from ase.io import write

    atoms = bulk("Si", "diamond", a=A0, cubic=True)
...
def spectrum_metrics(eigs_by_k: dict[str, np.ndarray], n_occ: int = N_OCC) -> dict:
...
def run_vasp_band(outdir: Path, nprocs: int, timeout: int, encut: float) -> dict:
    outdir.mkdir(parents=True, exist_ok=True)
    write_si_poscar(outdir / "POSCAR")
    shutil.copy(POT_ROOT / "Si" / "POTCAR", outdir / "POTCAR")
    (outdir / "INCAR").write_text(
        f"""SYSTEM = Si KS eigenvalue cross
PREC = Accurate
ENCUT = {encut}
EDIFF = 1E-7
NELM = 80
...
def run_openmx_band(outdir: Path, nprocs: int, timeout: int) -> dict:
    ...
    generate_input(
        ...
        dry_run=False,
        verbose=False,
        output_path=str(outdir / "Si_band.dat"),
    )
    text = (outdir / "Si_band.dat").read_text()
    text = re.sub(r"System\.Name\s+\S+", "System.Name        Si_band", text, count=1)
    ...
    if re.search(r"scf\.EigenvalueSolver", text, re.I):
        ...
        text += "\nscf.EigenvalueSolver        Band\n"
    (outdir / "Si_band.dat").write_text(text)

    t0 = time.time()
    inner = (
        "source /opt/intel/oneapi/setvars.sh >/dev/null 2>&1 || true; "
        "export PATH=/opt/intel/oneapi/mpi/latest/bin:$PATH; "
        f"mpirun -np {int(nprocs)} {OMX_BIN} Si_band.dat"
    )
    proc = subprocess.run(
        [
            _runner(), "exec",
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--outdir",
        type=Path,
        default=_REPO / "work" / "benchmarks" / "cross_band_si",
    )
    ...
    docs_dir = _REPO / "docs" / "benchmarks" / "cross_band_si"
    ...
    print("=== VASP special-k SCF ===")
    vasp = run_vasp_band(outdir / "vasp", args.np, args.timeout, args.encut)
    print(" VASP k:", vasp.get("k_found"), "gap:", vasp.get("gap_fundamental_eV"))
    print("=== OpenMX SCF eigenvalues ===")
    omx = run_openmx_band(outdir / "openmx", args.np, args.timeout)
    print(" OpenMX k:", omx.get("k_found"), "gap:", omx.get("gap_fundamental_eV"))

    cmp = compare_spectra(vasp, omx)
    gate = gate_result(cmp, args.tol_gap, args.tol_rms)
    report = {
        "kind": "cross_band_si",
        "protocol": {
            "structure": f"Si diamond cubic a0={A0} Å",
