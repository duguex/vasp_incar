...
This is the neutral, cross-code intermediate representation shared by every
DFT code plugin (VASP, OpenMX, ...). It lives in ``dft_utils`` — the base
layer both ``vasp_query`` and ``omx_tools`` depend on — so no single code
package owns the canonical IR.

``omx_tools.semantic.ir`` re-exports everything here for backward
